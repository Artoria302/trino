...
public class DistributedClusterStatsResource
{
    ...
    public Response getClusterStats()
    {
        ...
        long activeCoordinators = internalNodeManager.getNodes(NodeState.ACTIVE).stream()
        ...
        long totalAvailableProcessors = clusterStateProvider.getTotalAvailableProcessors();
    ...
    {
        MemoryPoolInfo memoryPoolInfo = clusterStateProvider.getClusterMemoryPoolInfo().getMemoryPoolInfo();
        return Response.ok()
                .entity(memoryPoolInfo)
                .build();
    }
