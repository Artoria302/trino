...
import io.trino.localcache.collect.Maps.IteratorBasedAbstractMap;
        ...
        implements RangeMap<K, V>
{
    ...
    @CheckForNull
    public Entry<Range<K>, V> getFloorEntry(K key)
    {
        Entry<Cut<K>, RangeMapEntry<K, V>> mapEntry =
                entriesByLowerBound.floorEntry(Cut.belowValue(key));
        if (mapEntry != null) {
    ...
    @CheckForNull
    public Entry<Range<K>, V> getCeilingEntry(K key)
    {
        Entry<Cut<K>, RangeMapEntry<K, V>> mapEntry =
                entriesByLowerBound.ceilingEntry(Cut.belowValue(key));
        if (mapEntry != null) {
            return mapEntry.getValue();
        }
        else {
            ...
            BiFunction<? super V, ? super @Nullable V, ? extends @Nullable V> remappingFunction)
    {
        ...
        entriesByLowerBound.putAll(gaps.buildOrThrow());
    }
    ...
    private final class AsMapOfRanges
            extends IteratorBasedAbstractMap<Range<K>, V>
    {
