...
public final class UserGroupInformationUtils
{
    ...
    public static <R, E extends Exception> R executeActionInDoAs(UserGroupInformation userGroupInformation, GenericExceptionAction<R, E> action)
            throws E
    {
        return userGroupInformation.doAs((PrivilegedAction<UserGroupInformationUtils.ResultOrException<R, E>>) () -> {
            try {
                return new UserGroupInformationUtils.ResultOrException<>(action.run(), null);
            }
            catch (Throwable e) {
                return new UserGroupInformationUtils.ResultOrException<>(null, e);
            }
        }).get();
    }
