...
{
    private static final Logger log = Logger.get(ExchangeFileSystemCache.class);

    public static final String CACHE_KEY = "fs.cache.credentials";

    public static final ExchangeFileSystemCache INSTANCE = new ExchangeFileSystemCache();

    private final AtomicLong unique = new AtomicLong();

    private final ExchangeFileSystemCacheStats stats;

    @GuardedBy("this")
    private final Map<ExchangeFileSystemCache.FileSystemKey, ExchangeFileSystemCache.FileSystemHolder> map = new HashMap<>();

    @VisibleForTesting
    ExchangeFileSystemCache()
    {
        this.stats = new ExchangeFileSystemCacheStats(() -> {
            synchronized (this) {
                return map.size();
            }
        });
            ...
            throws IOException
    {
        ...
        ExchangeFileSystemCache.FileSystemKey key = createFileSystemKey(uri, userGroupInformation, unique);
        Set<?> privateCredentials = getPrivateCredentials(userGroupInformation);

        ExchangeFileSystemCache.FileSystemHolder fileSystemHolder = map.get(key);
        if (fileSystemHolder == null) {
            ...
            try {
                ...
                fileSystemHolder = new ExchangeFileSystemCache.FileSystemHolder(fileSystem, privateCredentials);
            ...
            throws IOException
    {
        ...
        FilterFileSystem wrapper = new ExchangeFileSystemCache.FileSystemWrapper(original);
    ...
    public synchronized void closeAll()
            throws IOException
    {
        for (ExchangeFileSystemCache.FileSystemHolder fileSystemHolder : ImmutableList.copyOf(map.values())) {
            fileSystemHolder.getFileSystem().close();
        }
        map.clear();
    }

    private static ExchangeFileSystemCache.FileSystemKey createFileSystemKey(URI uri, UserGroupInformation userGroupInformation, long unique)
    {
        ...
        return new ExchangeFileSystemCache.FileSystemKey(scheme, authority, unique, realUser, proxyUser);
    }
    ...
    private static class FileSystemKey
    {
        ...
        public boolean equals(Object o)
        {
            ...
            ExchangeFileSystemCache.FileSystemKey that = (ExchangeFileSystemCache.FileSystemKey) o;
            ...
            extends FilterFileSystem
    {
        ...
        public FSDataInputStream open(Path f, int bufferSize)
                throws IOException
        {
            return new ExchangeFileSystemCache.InputStreamWrapper(getRawFileSystem().open(f, bufferSize), this);
        }
        ...
        public FSDataOutputStream append(Path f, int bufferSize, Progressable progress)
                throws IOException
        {
            return new ExchangeFileSystemCache.OutputStreamWrapper(getRawFileSystem().append(f, bufferSize, progress), this);
        }
        ...
        public FSDataOutputStream create(Path f, FsPermission permission, boolean overwrite, int bufferSize, short replication, long blockSize, Progressable progress)
                throws IOException
        {
            return new ExchangeFileSystemCache.OutputStreamWrapper(getRawFileSystem().create(f, permission, overwrite, bufferSize, replication, blockSize, progress), this);
        }
        ...
        public FSDataOutputStream create(Path f, FsPermission permission, EnumSet<CreateFlag> flags, int bufferSize, short replication, long blockSize, Progressable progress, Options.ChecksumOpt checksumOpt)
                throws IOException
        {
            return new ExchangeFileSystemCache.OutputStreamWrapper(getRawFileSystem().create(f, permission, flags, bufferSize, replication, blockSize, progress, checksumOpt), this);
        }
        ...
        public FSDataOutputStream createNonRecursive(Path f, FsPermission permission, EnumSet<CreateFlag> flags, int bufferSize, short replication, long blockSize, Progressable progress)
                throws IOException
        {
            return new ExchangeFileSystemCache.OutputStreamWrapper(getRawFileSystem().createNonRecursive(f, permission, flags, bufferSize, replication, blockSize, progress), this);
        }
    ...
    public ExchangeFileSystemCacheStats getFileSystemCacheStats()
    {
        return stats;
    }
}
