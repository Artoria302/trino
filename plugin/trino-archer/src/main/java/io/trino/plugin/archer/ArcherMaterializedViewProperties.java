...
public class ArcherMaterializedViewProperties
{
    ...
    @Inject
    public ArcherMaterializedViewProperties(ArcherConfig archerConfig, ArcherTableProperties tableProperties)
    {
        materializedViewProperties = ImmutableList.<PropertyMetadata<?>>builder()
                .add(stringProperty(
                        STORAGE_SCHEMA,
                        "Schema for creating materialized view storage table",
                        archerConfig.getMaterializedViewsStorageSchema().orElse(null),
                        false))
