...
public class ArcherTableName
{
    private ArcherTableName() {}
    ...
    public static boolean isArcherTableName(String tableName)
    {
    ...
    public static String tableNameFrom(String validArcherTableName)
    {
        Matcher match = TABLE_PATTERN.matcher(validArcherTableName);
        checkArgument(match.matches(), "Invalid Archer table name: %s", validArcherTableName);
        return match.group("table");
    }

    public static TableType tableTypeFrom(String validArcherTableName)
    {
        Matcher match = TABLE_PATTERN.matcher(validArcherTableName);
        checkArgument(match.matches(), "Invalid Archer table name: %s", validArcherTableName);
    ...
    public static boolean isDataTable(String validArcherTableName)
    {
        Matcher match = TABLE_PATTERN.matcher(validArcherTableName);
        checkArgument(match.matches(), "Invalid Archer table name: %s", validArcherTableName);
        String typeString = match.group("type");
    ...
    public static boolean isMaterializedViewStorage(String validArcherTableName)
    {
        return tableTypeFrom(validArcherTableName) == MATERIALIZED_VIEW_STORAGE;
    }
}
