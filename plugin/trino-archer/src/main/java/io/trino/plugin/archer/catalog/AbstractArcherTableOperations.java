...
import io.trino.plugin.archer.util.HiveSchemaUtil;
...
import static net.qihoo.archer.TableProperties.METADATA_COMPRESSION;
...
@NotThreadSafe
public abstract class AbstractArcherTableOperations
        implements ArcherTableOperations
{
    private static final Logger log = Logger.get(AbstractArcherTableOperations.class);

    protected static final String METADATA_FOLDER_NAME = "metadata";

    public static final StorageFormat ARCHER_METASTORE_STORAGE_FORMAT = StorageFormat.create(
    ...
    protected AbstractArcherTableOperations(
            FileIO fileIo,
    ...
    protected void refreshFromMetadataLocation(String newLocation, Function<String, TableMetadata> metadataLoader)
    {
        ...
        try {
            ...
            if (isNotFoundException(failure)) {
                throw new TrinoException(ARCHER_MISSING_METADATA, "Metadata not found in metadata location for table " + getSchemaTableName(), failure);
            }
            if (failure instanceof ValidationException) {
                throw new TrinoException(ARCHER_INVALID_METADATA, "Invalid metadata file for table " + getSchemaTableName(), failure);
            }
            throw failure;
        }
    ...
    private static boolean isNotFoundException(Throwable failure)
    {
        ...
        return failure instanceof net.qihoo.archer.exceptions.NotFoundException ||
    ...
    protected static String newTableMetadataFilePath(TableMetadata meta, int newVersion)
    {
        ...
        return metadataFileLocation(meta, format("%08d-%s%s", newVersion, randomUUID(), getFileExtension(codec)));
    }
