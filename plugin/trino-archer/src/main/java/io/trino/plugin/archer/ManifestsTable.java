...
import net.qihoo.archer.ManifestFile.PartitionFieldSummary;
        ...
        implements SystemTable
{
    ...
    public ManifestsTable(SchemaTableName tableName, Table archerTable, Optional<Long> snapshotId)
    {
    ...
    public ConnectorPageSource pageSource(ConnectorTransactionHandle transactionHandle, ConnectorSession session, TupleDomain<Integer> constraint)
    {
        ...
        return new FixedPageSource(buildPages(tableMetadata, archerTable, snapshotId.get()));
    }

    private static List<Page> buildPages(ConnectorTableMetadata tableMetadata, Table archerTable, long snapshotId)
    {
        PageListBuilder pagesBuilder = PageListBuilder.forTable(tableMetadata);

        Snapshot snapshot = archerTable.snapshot(snapshotId);
        if (snapshot == null) {
            throw new TrinoException(ARCHER_INVALID_METADATA, format("Snapshot ID [%s] does not exist for table: %s", snapshotId, archerTable));
        }

        Map<Integer, PartitionSpec> partitionSpecsById = archerTable.specs();

        snapshot.allManifests(archerTable.io()).forEach(file -> {
