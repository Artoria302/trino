...
public final class PartitionFields
{
    ...
    private static final Pattern ARCHER_BUCKET_PATTERN = Pattern.compile("bucket\\[(\\d+)]");
    private static final Pattern ARCHER_TRUNCATE_PATTERN = Pattern.compile("truncate\\[(\\d+)]");
    ...
    public static String fromIdentifierToColumn(String identifier)
    {
        if (QUOTED_IDENTIFIER_PATTERN.matcher(identifier).matches()) {
            ...
            // TODO: Enhance quoted identifiers support in Archer partitioning to support mixed case identifiers
        ...
        // Currently, all Archer columns are stored in lowercase in the Archer metadata files.
    ...
    private static String toPartitionField(PartitionSpec spec, PartitionField field)
    {
        ...
        Matcher matcher = ARCHER_BUCKET_PATTERN.matcher(transform);
        if (matcher.matches()) {
            return format("bucket(%s, %s)", name, matcher.group(1));
        }

        matcher = ARCHER_TRUNCATE_PATTERN.matcher(transform);
        if (matcher.matches()) {
