...
public class ArcherTransactionManager
{
    private final ArcherMetadataFactory metadataFactory;
    private final ClassLoader classLoader;
    private final ConcurrentMap<ConnectorTransactionHandle, MemoizedMetadata> transactions = new ConcurrentHashMap<>();

    @Inject
    public ArcherTransactionManager(ArcherMetadataFactory metadataFactory)
    {
        this(metadataFactory, Thread.currentThread().getContextClassLoader());
    }

    public ArcherTransactionManager(ArcherMetadataFactory metadataFactory, ClassLoader classLoader)
    {
    ...
    public ArcherMetadata get(ConnectorTransactionHandle transactionHandle, ConnectorIdentity identity)
    {
    ...
    {
        @GuardedBy("this")
        private ArcherMetadata metadata;

        public synchronized Optional<ArcherMetadata> optionalGet()
        {
            return Optional.ofNullable(metadata);
        }

        public synchronized ArcherMetadata get(ConnectorIdentity identity)
        {
            if (metadata == null) {
                try (ThreadContextClassLoader ignored = new ThreadContextClassLoader(classLoader)) {
