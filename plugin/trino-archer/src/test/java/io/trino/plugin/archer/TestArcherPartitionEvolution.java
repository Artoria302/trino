...
public class TestArcherPartitionEvolution
        extends AbstractTestQueryFramework
{
    ...
    @Test
    public void testDropPartitionData()
    {
        String tableName = "test_remove_partition_" + randomNameSuffix();
        assertUpdate("CREATE TABLE " + tableName + " WITH (partitioning = ARRAY['regionkey']) AS SELECT * FROM nation WHERE nationkey < 10", 10);
        List<MaterializedRow> files = computeActual("SELECT * FROM " + tableName + " where regionkey = 1").getMaterializedRows();
        assertThat(files).hasSize(3);
        assertUpdate("DELETE FROM " + tableName + " where regionkey = 1", 3);
        List<MaterializedRow> files1 = computeActual("SELECT * FROM " + tableName).getMaterializedRows();
        assertThat(files1).hasSize(7);
        assertUpdate("DROP TABLE " + tableName);
    }
}
